...
export const getApprovedGuides = async (req, res) => {
  try {
    ...
    const cacheKey = `guides_${destination || 'all'}_${language || 'all'}_${page}_${limit}`;
    
    // ⚡ PERFORMANCE: Check cache first
    const cachedData = appCache.get(cacheKey);
    if (cachedData) {
      console.log('✅ Returning cached guides data');
    ...
    // ⚡ PERFORMANCE: Cache the response (300 seconds = 5 minutes)
