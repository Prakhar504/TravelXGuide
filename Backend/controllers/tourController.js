...
import { appCache } from "../server.js";
...
export const getApprovedTours = async (req, res) => {
  try {
    ...
    // ⚡ PERFORMANCE: Create cache key
    const cacheKey = `approved_tours_${page}_${limit}`;
    ...
    const total = await tourModel.countDocuments({ status: 'approved' });
    ...
    // ⚡ PERFORMANCE: Cache the response (300 seconds)
    appCache.set(cacheKey, response, 300);
...
export const getAllTours = async (req, res) => {
  try {
    ...
    // ⚡ PERFORMANCE: Use lean() and limit field selection
...
export const updateTourStatus = async (req, res) => {
  try {
    ...
    // ⚡ PERFORMANCE: Invalidate cache when tour status changes
    appCache.flushAll();
